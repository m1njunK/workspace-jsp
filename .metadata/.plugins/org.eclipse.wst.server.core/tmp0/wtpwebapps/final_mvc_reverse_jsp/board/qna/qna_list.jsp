		...
		<c:choose>
			<c:when test="${!empty qnaList}">
						...
						<c:when test="${board.qna_delete eq 'N'}">
							<tr>
								...
								<td style="text-align:left;padding:5px;">
									...
									<a href="detail.qna?qna_num=${board.qna_num}">
									${board.qna_title}
									</a>
								</td>
				...
				<tr>
					<th colspan="8">
						<c:if test="${pm.first}">
							<a href="boardList.qna${pm.makeQuery(1)}">[&lt;&lt;]</a>
						</c:if>
						<c:if test="${pm.prev}">
							<a href="boardList.qna${pm.makeQuery(pm.startPage-1)}">[&lt;]</a>
						</c:if>
						<c:forEach var="i" begin="${pm.startPage}" end="${pm.endPage}">
							<c:choose>
								...
								<c:otherwise>
									<a href="boardList.qna${pm.makeQuery(i)}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${pm.next}">
							<a href="boardList.qna${pm.makeQuery(pm.endPage+1)}">[&gt;]</a>
						</c:if>
						<c:if test="${pm.last}">
							<a href="boardList.qna${pm.makeQuery(pm.maxPage)}">[&gt;&gt;]</a>
						</c:if>
					</th>
				</tr>
			</c:when>
