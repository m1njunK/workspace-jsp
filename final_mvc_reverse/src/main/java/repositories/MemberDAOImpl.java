...
public class MemberDAOImpl implements MemberDAO {
	...
	public MemberVO getMemberById(String id) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				member = new MemberVO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getInt(5),
	...
	public boolean checkMember(String id, String name) {
		...
		String sql = "SELECT * FROM mvc_member " + " WHERE id = ? AND name = ? AND joinYN = 'Y'";
		conn = DBCPUtil.getConnection();
